...
const backgroundImages = [    '../im/background1.jpg',    '../img/background2.jpeg',    '../img/background3.jpeg',];

function changeBackground() {
    if (currentBackground < backgroundImages.length - 1) {
        currentBackground++;
    } else {
        currentBackground = 0;
    }

    document.body.css.backgroundImage = `url(${backgroundImages[currentBackground]})`;
}
...
function game() {
    ...
    function planet() {
        ...
        ctx.drawImage(sprite, 0, 0, 200, 200, -45, -44, 90,90);
        ctx.restore();
    }
    ...
    function _asteroids() {
        ...
        for(var i = 0; i < asteroids.length; i++) {
            if (!asteroids[i].destroyed) {
                ...
                if (distance < (((asteroids[i].width/asteroids[i].size) / 10) - 4) + 50) {
                    gameOver = true;
                    playing  = false;
                    canvas.addEventListener('mousemove', action);
                }
